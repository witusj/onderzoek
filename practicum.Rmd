...
`ggplot` werkt met `layers`. Als eerste definieren we de databron (in ons geval `rawData`) en de x- en y-variabelen (`WoSitnu` en `WoOppnu`). De volgende `layer` Daarna is de `geom_jitter()` functie. Als we de punten zo maar in de grafiek zouden laten tekenen, zou er veel overlap zijn. Door wat `random noise` toe te voegen, worden alle punten zichtbaar en wordt de spreiding beter zichtbaar.

<br>
**Tussenvraag 3:** Verander `geom_jitter()` door `geom_point()` en bekijk het verschil.
